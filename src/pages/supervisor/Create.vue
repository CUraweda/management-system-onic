...
export default {
  ...
  methods: {
    ...
    async create() {
      try {
        const pic =
          this.selectedpic.label != undefined
            ? this.selectedpic.label
            : this.selectedpic.map((user) => user.label).join(",");
        ...
        this.addToForm(
          "priority",
          this.priority.value ? this.priority.value : this.priority
        );
        ...
        if (response.status === 200) {
          ...
          this.SpvApp
            ? this.$router.push({ path: "/supervisor/task_monitoring_2" })
